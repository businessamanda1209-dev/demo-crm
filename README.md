...
2. In **Project Settings -> API**, copy:
   - Project URL
   - anon public key
3. In **Project Settings -> Database**, copy the pooled Postgres connection string.
...
Create a user through the app first. Then copy that user's Supabase Auth user id and run:

```bash
SEED_USER_ID="supabase-user-id" npm run db:seed
```

The CRM data is scoped by `userId`, so each logged-in user only sees their own companies, contacts, and deals.

---

## Previous Railway Notes
