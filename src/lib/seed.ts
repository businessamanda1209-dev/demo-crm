...
export async function runSeed(prisma: PrismaClient, userId: string) {
  await prisma.deal.deleteMany({ where: { userId } });
  await prisma.contact.deleteMany({ where: { userId } });
  await prisma.company.deleteMany({ where: { userId } });

  const companies = await Promise.all(
    companiesData.map((c) => prisma.company.create({ data: { ...c, userId } })),
  );
  ...
  return {
    ...
    deals: await prisma.deal.count({ where: { userId } }),
  };
}
