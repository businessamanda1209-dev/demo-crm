...
export default function CompaniesView({
  ...
  async function remove(id: string) {
    if (!confirm(co.confirmDelete)) return;
    const res = await fetch(`/api/companies/${id}`, { method: "DELETE" });
    if (res.ok) {
      setCompanies((prev) => prev.filter((c) => c.id !== id));
      router.refresh();
    } else {
      alert(co.deleteError);
    }
  }
  ...
  return (
    <div>
      <PageHeader
        title={co.title}
        subtitle={co.subtitle(companies.length, formatCurrency(totalRevenue), formatNumber(totalEmployees))}
        actions={
      ...
      <div className="card overflow-hidden">
        <div className="flex flex-wrap items-center gap-2 px-4 py-3 border-b border-slate-100 dark:border-neutral-800">
          ...
          >
            <option value="ALL">{co.allIndustries}</option>
            {industries.map((i) => (
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full">
            <thead className="bg-slate-50 dark:bg-neutral-900/60 border-b border-slate-200 dark:border-neutral-800">
              <tr>
                <th className="table-th">{co.colName}</th>
                <th className="table-th">{co.colIndustry}</th>
                <th className="table-th">{co.colLocation}</th>
                <th className="table-th">{co.colEmployees}</th>
                <th className="table-th">{co.colRevenue}</th>
                <th className="table-th">{co.colContacts}</th>
                <th className="table-th">{co.colDeals}</th>
                <th className="table-th text-right">{co.colActions}</th>
              </tr>
            </thead>
      ...
      <Modal
        ...
        title={form.id ? co.modalEdit : co.modalNew}
      >
        <form onSubmit={submit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="label">{co.industry}</label>
              <input
                className="input"
                required
                value={form.industry}
            ...
            <div>
              <label className="label">{co.website}</label>
              <input
                className="input"
                placeholder="https://..."
                value={form.website}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="label">{co.employees}</label>
              <input
                className="input"
                type="number"
                min={0}
            ...
            <div>
              <label className="label">{co.annualRevenue}</label>
              <input
                className="input"
                type="number"
                min={0}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="label">{co.city}</label>
              <input
                className="input"
                value={form.city}
            ...
            <div>
              <label className="label">{co.country}</label>
              <input
                className="input"
                value={form.country}
          ...
          <div className="flex items-center justify-end gap-2 pt-2">
            ...
            <button type="submit" className="btn-primary" disabled={submitting}>
              {submitting ? co.saving : form.id ? co.saveChanges : co.createCompany}
            </button>
          </div>
        </form>
      </Modal>
